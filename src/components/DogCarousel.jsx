...
const CustomNavigation = ({ color, swiperRef }) => {
    return (
        <div className="absolute left-0 top-1/2 w-full flex z-10">
            <button onClick={() => swiperRef.current?.slidePrev()} className="absolute h-6 w-6 md:h-10 md:w-10 left-2 transform -translate-y-1/2 border-0">
            ...
            <button onClick={() => swiperRef.current?.slideNext()} className="absolute h-6 w-6 md:h-10 md:w-10 right-2 transform -translate-y-1/2 border-0">
                <svg className='w-full h-full' viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const DogCarousel = ({ backgroundImage, input, color }) => {
    ...
    return (
        <div className='pb-5 px-10 md:px-[3.5rem] relative'>
            <Swiper
            ...
            >
                {input.map((slide, index) => (
                    <SwiperSlide key={index} className='flex justify-center h-auto my-4 px-2'>
                        <DogCard
            ...
            <CustomNavigation color={color} swiperRef={swiperRef} />
        </div>
    );
};
